...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 18 21:14:42 2019
...
root_dir = "/Users/clin/Desktop/era/kanji"
g_list =[]
d_list =[]
d_acc_list = []
    ...
    def train(self, iterations, batch_size=128, save_interval=50, model_interval=1000, check_noise=None, r=5, c=5):
        ...
        for iteration in range(iterations):
            ...
            if iteration % save_interval == 0:
                ...
                cv2.imwrite("images/latent/" + "latentTwo_{}.png".format(iteration), resultImage)
    ...
    def save_imgs(self, iteration, check_noise, r, c):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i, j].axis('off')
                cnt += 1
        fig.savefig('images/gen_imgs/learningTwo_%d.png' % iteration)
...
if __name__ == '__main__':
    ...
    dcgan.train(iterations=6001, batch_size=32, save_interval=100, model_interval=5000, check_noise=check_noise, r=r,
                c=c)
    
#%%
#loss function outputer
itr_list = []
for itr in range(0,3877):
    itr_list.append(itr)

plt.figure(figsize=(15,5))
plt.plot(itr_list, d_list, label="discrminator's loss", linewidth=0.8)
plt.plot(itr_list, g_list, label="generator's loss", linewidth=0.8, color='lightcoral')
plt.title('Loss of Discriminator & Generator Graph')
plt.xlabel('iteration')
plt.ylabel('loss')
plt.legend(loc='lower right', borderaxespad=2, facecolor='white', shadow=True)

plt.style.use('seaborn-pastel')
plt.savefig('loss3877_1428.png')
plt.show()


#%%
plt.figure(figsize=(15,5))
plt.plot(itr_list, d_acc_list, label="discrminator's accuracy", linewidth=0.8)
g_acc_list = [ 1-x for x in d_acc_list]
plt.plot(itr_list, g_acc_list, label="generator's accuracy", linewidth=0.8, color='lightcoral')
plt.title('Accuracy of Discriminator & Generator Graph')
plt.xlabel('iteration')
plt.ylabel('accuracy percentage')
plt.legend(loc='lower right', borderaxespad=2, facecolor='white', shadow=True)

plt.style.use('seaborn-pastel')
plt.savefig('accuracy3877_1428.png')
plt.show()